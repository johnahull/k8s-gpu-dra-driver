...
func GetDriverVersion() string {
	...
	if len(matches) == 0 {
		glog.Warningf("No AMD GPU cards found for driver version reading, using fallback")
		return "0.0.0"
	}
	...
	glog.Warningf("Failed to read AMDGPU driver version from any card, using fallback")
	return "0.0.0"
}
